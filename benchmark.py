...
from optCuLegKan.layer import BetterLayer as optLayer
from original.LegKanLayer import KAL_Layer

from tqdm import tqdm


class optCuNet(nn.Module):
    def __init__(self, layers: Tuple[int, int, int], device: str):
        super().__init__()
        self.layer1 = optLayer(layers[0], layers[1], 9).to(device)
        self.layer2 = optLayer(layers[1], layers[1], 9).to(device)
        self.layer3 = optLayer(layers[1], layers[1], 9).to(device)
        self.layer4 = optLayer(layers[1], layers[1], 9).to(device)
        self.layer5 = optLayer(layers[1], layers[2], 9).to(device)
...
def main():
    ...
    if args.method == 'opt' or args.method == 'all':
        model = optCuNet(layers=[args.inp_size, args.hid_size, 1], device='cpu')
        model.to('cuda')
        res['opt-cuda-gpu'] = benchmark(dataset, 'cuda', args.batch_size, loss_fn, model, args.reps)
        res['opt-cuda-gpu']['params'], res['opt-cuda-gpu']['train_params'] = count_params(model)
